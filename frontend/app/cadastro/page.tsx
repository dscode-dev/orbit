...
const steps = [
  ...
  { title: "Organização", description: "Identidade do workspace", icon: Building2 },
...
export default function CadastroPage() {
  ...
  const {
    ...
    defaultValues: { primarySegment: "Serviços", stateCode: "PE", terms: false },
  });

  async function next() {
    if (await trigger(fieldsByStep[step])) setStep((current) => Math.min(current + 1, 2));
  }
      ...
      <aside className="bg-gradient-orbit relative hidden overflow-hidden p-8 text-primary-foreground lg:flex lg:flex-col">
        ...
        <div className="relative my-auto">
          ...
          <p className="mt-3 text-sm leading-relaxed text-primary-foreground/75">
            Configure a organização em poucos passos. Você entra como owner e pode convidar o time
            depois.
          </p>
              ...
              return (
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="block text-xs text-primary-foreground/65">{item.description}</span>
                  </span>
                </li>
              );
            })}
          ...
          <form onSubmit={handleSubmit(submit)} className="w-full">
            <div className="mb-8 flex gap-2 lg:hidden">
              {steps.map((item, index) => (
                <span
                  key={item.title}
                  className={cn("h-1 flex-1 rounded-full bg-muted", index <= step && "bg-primary")}
                />
              ))}
            </div>
              ...
              >
                <div className="mb-8">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    {step === 0 && "Crie suas credenciais de acesso administrativo."}
                    {step === 1 && "Dê um nome ao ambiente que sua equipe verá todos os dias."}
                    {step === 2 && "Cadastre a matriz para contextualizar sua operação."}
                  </p>
                </div>
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    {(["password", "confirmPassword"] as const).map((name) => (
                      <div key={name} className="space-y-2">
                        <Label htmlFor={name}>{name === "password" ? "Senha" : "Confirmar senha"}</Label>
                ...
                {step === 1 && (
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="documentNumber" inputMode="numeric" {...register("documentNumber")} />
                      <FieldError message={errors.documentNumber?.message} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="primarySegment">Segmento principal</Label>
                      <Input id="primarySegment" {...register("primarySegment")} />
                ...
                {step === 2 && (
                  <div className="grid gap-5 sm:grid-cols-[1fr_6rem]">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="stateCode" maxLength={2} className="uppercase" {...register("stateCode")} />
                    ...
                    <div className="rounded-xl border border-border bg-muted/40 p-4 sm:col-span-2">
                      <div className="flex items-start gap-3">
                        ...
                        <Label htmlFor="terms" className="text-sm font-normal leading-relaxed">
                          Concordo com os Termos de Uso e a Política de Privacidade do Orbit.
                        </Label>
                      </div>
