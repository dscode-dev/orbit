...
module.exports = async function cleanupE2eTenants() {
  ...
  try {
    ...
    const users = await client.query(`
      SELECT id
        FROM users
       WHERE email ~ '^(manifest\\.e2e\\.|render\\.e2e\\.|portal\\.e2e\\.|inv\\.|quotes\\.|conc\\.|financial\\.|rep\\.|rls\\.|auto\\.|pmoc\\.|professional\\.|evidence\\.|rvt\\.|qr\\.|assignment\\.|mobile\\.sign\\.|push\\.|offline\\.|mobile\\.field\\.|field\\.execution\\.|artifact\\.|analytics\\.|entitlements\\.)'
